            ...
            <div class="left_panel_content">
                <div class="category">
                    ...
                    <div class="category_list">
                        <div id="categoryMenu" 
                            style="display:none; position:absolute; border:1px solid #ccc; background:#fff; z-index:1000;">
                        ...
                        <a href="login_success.jsp?categoryId=<%= listIdx %>" class="category_item <%= selectedClass %>" 
                            data-id="<%=listIdx%>" data-name="<%=listName%>" style="text-decoration: none;">
            ...
            <div class="note_header">
                ...
                <label>배경색:
                    <select name="backgroundColor" id="memoColorSelect">
                        ...
                        <%= "default".equals(memoBackgroundColor) ? "selected" : "" %>>
                        기본
                        </option>
                        <option value="skyBlue"
                        <%= "skyBlue".equals(memoBackgroundColor) ? "selected" : "" %>>
                        하늘색
                        </option>
                        <option value="Yellow"
                        <%= "Yellow".equals(memoBackgroundColor) ? "selected" : "" %>>
                        흰색
                        </option>
                    </select>
                </label>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 1) 우클릭 잡기
    ...
    document.getElementById('editCategory').addEventListener('click', () => {
        const newName = prompt("카테고리 이름을 입력하세요", selName);
        ...
        // 서버에 전송
        fetch("updateCategoryName.jsp", {
            method: "POST",
            headers: { "Content-Type":"application/x-www-form-urlencoded" },
            body: `listIdx=${selId}&newName=${encodeURIComponent(newName)}`
        })
        .then(() => {
            window.location.href = 'login_success.jsp?categoryId=' + selId;
        })
